 ...
 *    출력 return:[5, 6, 3]
 */
public class Knth_number_2 {
	public int[] solution(int[] array, int[][] commands) {
    	int[] answer = new int[commands.length];
    	
        for (int i=0; i<commands.length; i++) {
        	int[] sub_arr = new int[commands[i][1]-commands[i][0]+1];
        	System.arraycopy(array, commands[i][0]-1, sub_arr, 0, sub_arr.length);
        	Arrays.sort(sub_arr);
	...
	public static void main(String[] args) {
		Knth_number_2 knth_number_2 = new Knth_number_2();
		int[] array = {1, 5, 2, 6, 3, 7, 4};
		int[][] commands = {{2, 5, 3}, {4, 4, 1}, {1, 7, 3}};
		knth_number_2.solution(array, commands);
	}
}
